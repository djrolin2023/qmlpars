...
function stopCam(){ if(stream){ stream.getTracks().forEach(t=>t.stop()); stream=null; } if(scanTimer){ clearInterval(scanTimer); scanTimer=null; } if(lightTimer){ clearInterval(lightTimer); lightTimer=null; } scanning=false; scanState='auto'; updateScanBtn(); }
// auto | paused | manual
