/* ============ 输入查询 ============ */
const PLATE_RE_FULL=/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领][A-Z][A-Z0-9]{5,6}$/;
async function queryPlate(){
  ...
  if(!PLATE_RE_FULL.test(plate) && plate.length<4){ err.textContent='请输入完整车牌，或至少 4 位连续车牌字符（如 mxy5）'; return; }
  showLoading();
  try{
    ...
    else showResult(plate, { vehicle:null, isInternal:false, notFoundText:'【'+esc(plate)+'】暂无数据<br>请联系管理人员确认是否报备该车辆' });
